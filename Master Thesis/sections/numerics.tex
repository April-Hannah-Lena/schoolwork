...
\subsubsection{Validation of Koopman Eigenpairs}
...
\begin{equation}
    ...
    \begin{split}
        ...
        &= \left\| (P \scrL P - \bar{\lambda} I) g \right\|_{\scrX}^2
    \end{split}
\end{equation}
where $P$ is the orthogonal projector onto $\spn \left\{ \psi_j \right\}_{j=1}^N$. By 
the Galerkin property we know that $L$ encodes precisely the action of $P \scrL P$. Hence, 
\begin{equation}
    \label{eq:PLP}
    \left\| (P \scrL P - \bar{\lambda} I) g \right\|_{\scrX}^2
...
\todo{replace $c$ with $v$ when $M$-dimensional}
...
\begin{proof}
    ...
    \begin{equation}
        ...
        = K P_{\ran\, \YX^* \sqrt{W}}
    \end{equation}
    where $P_{\ran \YX^*}$ is the orthogonal projection onto the range of $\YX^*$. 
...
features $g = \Psi \cdot (Z c)$ the same way as in theorem \ref{thm:res_M_limit}. However, 
...
\begin{proposition}[\cite{resdmd}]
    ...
    for any eigenpair $(\lambda, c)$ of $\widehat{K}$, the residual
    \begin{equation}
        \res_{N,M} (\lambda, Z c) = 0 . 
    \end{equation}
\end{proposition}
...
\begin{proof}
    Write
    \begin{equation}
        \sqrt{W} (\YY - \lambda \YX) (Z c) 
        = \sqrt{W} \YY Z c - \lambda Q \Sigma c
        = \sqrt{W} \YY \YX^* \sqrt{W} Q \Sigma^\dagger c - \lambda Q \Sigma c . 
    \end{equation}
    ...
    \begin{equation}
        \sqrt{W} (\YY - \lambda \YX) (Z c) = Q \Sigma (\widehat{K} - \lambda I) c = 0 .  
    \end{equation}
\end{proof}
...
error is given by 
\begin{equation}
    \widehat{ \res }_{N,M} (\lambda, c) 
    = \left\| \left( \widehat{ \YY } - \lambda \widehat{ \YX } \right) c \right\| . 
\end{equation}
To provide a computable formulation we write
\begin{equation}
    \widehat{ \res }_{N,M} (\lambda, c)^2
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \State Compute $\widehat{ \res } (z_\nu) 
            \defeq \min_{\| c \| = 1} \widehat{ \res } (z_\nu, c)$, which is equivalent 
...
\begin{equation}
    \begin{split}
        ...
        &= Z^* A^* G^{-1} Z \\
        &= Z^* G G^{-1} A^* G^{-1} Z \\
        &= \left( Z^* G \right) P \left( G^{-1} Z \right) \\
        &= \left( \Sigma^2 Z^* \right) P \left( \Sigma^2 Z^* \right)^\dagger
    \end{split}
\end{equation}
...
\begin{equation}
    \widehat{ \res }_{N,M} (\lambda) = \widetilde{ \res }_{N,M} (\bar{\lambda}) 
\end{equation}
so that algorithm \ref{alg:kresdmd} computes precisely the pseudospectrum of $P \scrL P$ 
as in \ref{eq:PLP}. 
