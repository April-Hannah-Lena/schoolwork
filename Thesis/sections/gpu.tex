...
\begin{definition}
    ...
    maximum number of threads $N$ per SM allowed by the hardware the \emph{occupancy $O$}, 
    that is, $O = n / N$.
\end{definition}
...
The precise "best" balance between occupancy and registers is highly problem dependent, 
...
and block counts to approximately maximize occupancy while attempting to ensure that 
threads have large enough register availability. The julia package \jlinl{CUDA.jl} 
