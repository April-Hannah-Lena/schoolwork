...
A natural question to ask is whether the vector \jlinl{x = [0.5, 0.6]} lies within 
...
This section is meant for those interested in the inner structure and implementation of 
\texttt{GAIO.jl}. All descriptions here can also be found in the docstrings, eg. by typing 
\jlinl{julia> ? Box} into the julia REPL.

The package is built off of the base structures \jlinl{Box, BoxPartition, BoxMap}:
...
$ A Box is a half-open, generalized rectangle, that is, product of half-open intervals
...
for a box grid and only store the dimensions of the grid:
...
Finally, we need a way to convert a map
$f$ defined on $Q$ to a map \jlinl{F} defined on boxes of a \jlinl{BoxPartition}:
...
points that are mapped by the given function $f$. This is \jlinl{domain_points}.
